...
var (
    // Метаданные из переменных окружения Knative
...
func init() {
    ...
    // Зарегистрировать cold start при запуске
...
func main() {
    ...
    // Основной endpoint для тестирования биллинга
    r.Any("/invoke", invokeHandler)
    r.Any("/api/v1/metrics", invokeHandler) // Совместимость с echo сервисом
    
    port := getEnv("PORT", "8080")
    log.Printf("Waiter service starting on :%s", port)
...
func invokeHandler(c *gin.Context) {
    ...
    // Определить cold start (первый запрос после старта или долгого простоя)
    ...
    if req.MemMB > 0 {
        ...
        // Запись в буфер для реального выделения памяти
...
// Простая CPU-intensive функция для тестирования
